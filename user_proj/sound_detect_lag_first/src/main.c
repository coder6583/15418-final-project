...
void sounddetect_ref_pair(
  ...
  int16_t audio_data[NUM_SAMPLES];

  // Simplifying from 24 bits to 16 bits
  for (int i = 0; i < NUM_SAMPLES; i++) {
    audio_data[i] = (int16_t)(raw_data[i + offset][rank] >> 8);
  }

  int16_t ref_data[NUM_SAMPLES];
  if (rank == 0) {
    for (int i = 0; i < NUM_SAMPLES; i++) {
  ...
  *total_computation_time += (get_time() - comp_start);
  ...
  tinimpi_bcast((uint8_t*)ref_data, NUM_SAMPLES * sizeof(int16_t), 0);
  *total_communication_time += (get_time() - comm_start);
  ...
  struct TimeLag best_dist_diffs[TOPOLOGY];
  ...
  *total_computation_time += (get_time() - comp_start);
  ...
  tinimpi_send2(next_rank, 0, (uint8_t*)&timelag_result, sizeof(struct TimeLag));
  int done = 0;
  while (!done) {
    struct TimeLag recv_buffer;
    uint16_t out_len;
    tinimpi_recv2(prev_rank, 0, (uint8_t*)&recv_buffer, sizeof(struct TimeLag), &out_len);

    int orig_src = recv_buffer.src;
    if (orig_src == rank) {
      done = 1;
    } else {
      memcpy(&best_dist_diffs[orig_src], &recv_buffer, sizeof(struct TimeLag));
      tinimpi_send2(next_rank, 0, (uint8_t*)&recv_buffer, sizeof(struct TimeLag));
      done = 0;
    }
  }
  tinimpi_barrier2();
